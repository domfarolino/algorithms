...
//
// Simple and most widely known encryption of the encryption techniques. Caesar cipher is a
// substitution cipher where each letter in plain text is 'shifted' a certain number of
// letters in the alphabet. This cipher offers no communication security and can easily
// be broken by pen and paper
//
...
std::string updateCipher(std::string cipher, const std::function<char(char)> func) {
  char character;
...
void testEncryption(const std::string cipher, const int& shiftAmount){
  std::cout << "Encrypting: \n\t\"" << cipher << "\" shifted: " << shiftAmount << std::endl;
