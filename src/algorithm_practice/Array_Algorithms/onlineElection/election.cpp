 ...
 *     overhead.
 * 2.) Is the times vector sorted? Yes, and this doesn't have too much of an impact on design.
 * 3.) Will we be given a time query >= every value in the times vector? Yes, and same note as above.
 *
 * Some approaches:
 *
 * 1.) No preprocessing at all
 ...
 *     in this subarray, so we can return the winner up to this point. To store the counts of each
 *     candidate vote up to this point in our subarray, we can create a map. Populating this map with
 *     candidates and their respective vote counts will be O(n), we'll then need to iterate through the
 *     map, and choose the key whose value is the highest, also O(n). A small optimization we could make
 *     is to keep track of the maximum key as we insert candidates into the map, but we're still bounded by
 *     the O(n) subarray traversal for each query.
 *
 *     Complexity analysis
 ...
 *
 * 2.) Preprocessing
 *     With some preprocessing, we can get our queries down to O(log(n)). The realization we need to make is
 *     that we can extend the logic from the previous section to each possible time query up front. If we have
 *     the candidate count and max candidate for a given query, and thus the winning candidate at a given position,
 *     we can easily find the winning candidate for the next position, by consuming the next candidate, incrementing
 *     their vote count, and seeing if it overtakes or is equal to our set-aside max candidate. We can store each of
 *     these winning candidates at each position in a separate query array the same size of the candidate person array,
 *     and query it in O(1) time after we get our O(log(n)) time. The time index we care about will be the same index of
 *     of the value we want in our query array of course. There are some more minor optimizations, but as George Costanza
 *     says, "that's the gist of it!".
 *
 *     Complexity analysis
