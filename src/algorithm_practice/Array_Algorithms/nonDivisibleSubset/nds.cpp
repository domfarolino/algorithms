 ...
 * only if n1%k = x, and n2%k = 1-x. The two operands % k must have complementary modulo values. The next
 * key realization that I did not personally get is noting we should maintain a map where the keys are the
 * possible values of any number % k. The values of this map will be the number of times each value in the
 * array % k exists. So given the array [1, 5, 11], k = 6 we'd have a map looking like this: {1: 1, 5: 2} because
 * there is one value such that value % k = 1, and two values such that value % k = 5. Again, we know that the only
 * two values that can be summed to form a multiple of k are values whose modulo of k are complementary. This means
 * n1 % k = x, n2 % k = k - x. In this case, we need to look at exactly how many values share the key x, and k - x.
 * The number of values we'll need to get rid of in the array given some pair of x, k - x will be the smaller of the
 * two uMap[x] and uMap[k - x]. In the above example getting rid of 1 is cheaper than getting rid of both 5 and 11  because
 * the 5 and 11 don't interfere with each other, but the 1 interferes with both. We can leave the two values whose modulo with
 * k is 5 and still meet the same requirements. We need to do this for every x, k - x key pair and keep track of how many values
 * we eliminate. This means we can effectively iterate up through k/2 so we can be assured we don't duplicate pairwise work.
 *
...
long largestSubset(vector<long>& v, long k) {
  ...
  for (long i = 1; i <= k/2; ++i) {
    numberOfValuesRemoved += min(uMap[i], uMap[k - i]);
  }
