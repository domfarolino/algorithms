...
The approach I took for this problem was to pick the lexico[...] cheapest distinct character,
...
character that appears after `idx`, but before the last instance of all remaining distinct characters.
Since we're iterating in alphabetical order, once we find one, we know we've found the best one. Record
it to our "return subsequence" string, and remove it from the list of "remaining distinct characters".
...
1. We need to see if this instance (which is > `idx`), also comes before the last-indices of all remaining
   distinct characters. If it does, we know all remaining characters appear after this one, and we can choose it
...
as well, giving us an O(n<sup>2</sup>) operations. However, since we're performing binary search for each inner
